...
export const addFolderToDB = (e, folderName, setFolderName) => {
  ...
  if (folderName === "" || folderName.startsWith(" ")) {
    ...
    const databaseRef = collection(
      database,
      `/Gallary/FoldersName/${sessionStorage.getItem("uid")}/`
    );
    addDoc(databaseRef, {
...
export const deleteFolderImages = (folderUrl, images) => {
  images.forEach((image) => {
    const urlRef = doc(
      database,
      `/Gallary/Images/${sessionStorage.getItem("uid")}/${folderUrl}/images`,
      image.id
    );
    deleteDoc(urlRef)
      .then(() => {})
  ...
  const storageRef = ref(
    storage,
    `/GlobalImages/${sessionStorage.getItem("uid")}/${folderUrl}`
  );
