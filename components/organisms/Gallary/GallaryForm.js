...
function GallaryForm({ folderName }) {
    ...
    return (
        <>
            <Para className={`fixed top-0 -ml-4 w-full text-center py-3 bg-green-500 text-white ${successUpload ? '' : 'hidden'}`}>
            ...
            <div className="w-full fixed top-18 md:top-20 flex gap-4 justify-end md:justify-center flex-wrap z-102 py-2.5 md:py-4 px-2 bg-gradient-to-r from-black via-gray-900 to-slate-800 -ml-4">
                <Link href="/folders/navigation" className="fixed left-1.5 top-19 md:top-24 z-102 text-slate-800 flex gap-2 items-center px-4 py-2 rounded-full bg-slate-100 text-sm">
                ...
                <button className="px-4 py-1 border border-white rounded-md" onClick={isOpenSearch}>
            ...
            <div
                className={`fixed right-0 top-2/4 -translate-y-2/4 px-6 pt-3 rounded-xl bg-white h-3/4 z-105 ${toggleUpload ? 'block' : 'hidden'
                    }`}
            >
                        ...
                        targetFiles.map((item) => (
                            <div key={item.name} className="">
                                <Para className="text-ellipse">
                                    {item.name === undefined ? <span></span> : item.name.slice(0, 60)}
                                </Para>
                                <Para
                ...
                <div className="flex gap-2 lg:gap-5">
                    <Button
                        className={`${styles.button_sm} ${[...file].length === success ? styles.button_purple : 'bg-slate-400'}`}
                    ...
                    <Button
                        className={`${styles.button_sm} ${[...file].length === success ? 'bg-gray-600 ml-4 cursor-none' : styles.button_purple}`}
